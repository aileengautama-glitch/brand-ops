...
export default function EventBudget() {
  ...
  const removeBudgetItem = useEventStore((s) => s.removeBudgetItem)

  const { canEdit } = useCurrentUser()
  const readOnly = !canEdit('event.budget', id)
  ...
  return (
    <div className="p-6 max-w-4xl">
      ...
      <PageSection label="Line Items" card>
        {project.budgetItems.length === 0 ? (
          <EmptyState icon={Wallet} title="No budget items yet"
            description="Add line items to track your spend."
            action={!readOnly ? (
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => { setShowAdd(false); setDraft(BLANK_DRAFT) }}
        ...
        <div className="space-y-3">
          <FormField label="Description" required>
            <input autoFocus type="text" value={draft.description}
              ...
              placeholder="e.g. Venue hire" className={inputCls} />
          </FormField>
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Category">
              <input type="text" value={draft.category}
                onChange={(e) => d('category', e.target.value)}
                placeholder="e.g. Venue" className={inputCls} />
            </FormField>
            <FormField label="Supplier">
              <input type="text" value={draft.supplier}
                onChange={(e) => d('supplier', e.target.value)} className={inputCls} />
            </FormField>
            <FormField label="Estimated cost">
              <input type="number" value={draft.estimatedCost}
