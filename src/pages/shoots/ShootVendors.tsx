...
export default function ShootVendors() {
  ...
  const addVendor = useShootStore((s) => s.addVendor)
  const updateVendor = useShootStore((s) => s.updateVendor)
  const removeVendor = useShootStore((s) => s.removeVendor)
  ...
  const readOnly = !canEdit('shoot.vendors', id)
  ...
  return (
    <div className="p-6 max-w-4xl">
        ...
        {project.vendors.length === 0 ? (
          <EmptyState icon={Building2} title="No vendors yet"
            action={readOnly ? undefined : (
              <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-accent text-white rounded hover:bg-accent-dark transition-colors"><Plus size={13} /> Add vendor</button>
            )} />
        ) : (
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setDraft(BLANK) }} title="Add Vendor / Supplier"
        footer={
          <>
            ...
            <button onClick={handleAdd} disabled={!draft.name.trim()} className="px-3 py-1.5 text-sm bg-accent text-white rounded hover:bg-accent-dark transition-colors disabled:opacity-40">Add vendor</button>
          </>
        }
      >
        <div className="space-y-3">
          <FormField label="Name" required>
            <input autoFocus type="text" value={draft.name} onChange={(e) => d('name', e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleAdd()} className={inputCls} />
          </FormField>
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Category"><input type="text" value={draft.category} onChange={(e) => d('category', e.target.value)} placeholder="e.g. Studio, Equipment" className={inputCls} /></FormField>
            <FormField label="Status">
              <select value={draft.status} onChange={(e) => d('status', e.target.value as VendorStatus)} className={inputCls}>
                {(Object.entries(VENDOR_STATUS_LABELS) as [VendorStatus, string][]).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </FormField>
            <FormField label="Contract">
              <select value={draft.contractStatus} onChange={(e) => d('contractStatus', e.target.value as ContractStatus)} className={inputCls}>
                {(Object.entries(CONTRACT_STATUS_LABELS) as [ContractStatus, string][]).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </FormField>
            <FormField label="Contact info"><input type="text" value={draft.contactInfo} onChange={(e) => d('contactInfo', e.target.value)} placeholder="Email · Phone" className={inputCls} /></FormField>
          </div>
